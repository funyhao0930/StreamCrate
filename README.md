...
StreamCrate 是一個為 Windows 10 22H2 與 Windows 11 x64 設計的本機優先影片下載器。它以 [yt-dlp](https://github.com/yt-dlp/yt-dlp)、Deno 與 FFmpeg 處理使用者有權下載的公開或已登入媒體，提供 MP4、MP3、播放清單、FIFO 佇列、可續傳下載與本機歷史紀錄。

> 請只下載你有權存取與保存的內容。StreamCrate 不提供 DRM 規避功能，也不保證所有網站永遠可用。

## V1.0.2

- 新增自訂背景圖片與頁面轉場效果，提升介面個人化與視覺一致性。
- 下載佇列改為依等待中、下載中、已完成與失敗分區顯示，狀態更清楚。
- 下載路徑改用 Windows 資料夾選擇器，並保留使用者的路徑設定。
- 修正歷史紀錄項目的進場動畫。
- 更新 Windows x64 自包含安裝程式，安裝版本標示為 1.0.2。

## V1.0.0

- 聚焦畫布介面：新增下載、下載佇列、歷史紀錄與設定各自獨立，支援淺色／深色主題。
- 播放清單可逐項勾選、依原始順序加入 FIFO 佇列，並以播放清單名稱建立資料夾；名稱中的 Windows 非法字元會安全處理。
- 內建官方 yt-dlp nightly、Deno 與 FFmpeg 的驗證下載流程，改善近期 YouTube 串流相容性。
- 公開內容預設不使用 Cookie；登入內容的 Cookie 僅在本次操作期間使用，不會寫入設定、歷史或日誌。

## 功能

- 左側導覽：新增下載、下載佇列、歷史紀錄、設定
- 單一影片與播放清單解析；播放清單可個別取消勾選
- 播放清單輸出為 `播放清單名稱\01 - 影片標題 [影片ID].mp4`，不會依賴個別影片頁面缺失的播放清單欄位
- MP4（最佳、2160p、1440p、1080p、720p）與高品質 MP3
- 一次一項的 FIFO 下載、取消、失敗重試與 `.part` 續傳
- 公開內容預設不使用 Cookie；登入內容可使用 Firefox，或僅限本次使用的 Netscape `cookies.txt`。Chrome／Edge 在 Windows 可能受 App-Bound Encryption 限制
- 繁體中文／English、淺色／深色主題
- 無遙測；Cookie、授權標頭與可能含 token 的 URL 查詢字串不會寫入歷史或日誌

## 安裝與使用

從 GitHub Releases 下載 x64 安裝程式後執行。首次啟動時，程式會說明並要求同意下載 yt-dlp nightly、Deno 與 LGPL 版 FFmpeg；工具會從官方 Release 下載並驗證 SHA-256，保存到 `%LocalAppData%\StreamCrate`，媒體檔案則儲存在你選擇的資料夾。yt-dlp 使用官方 nightly build，以跟上 YouTube 經常變動的串流介面。
...
本專案以 [MIT License](LICENSE) 授權。yt-dlp、Deno、FFmpeg 與其相依元件各自依其原始授權條款提供。
