...
public sealed partial class MainWindow : Window
{
    ...
    private async void ProbeClicked(object sender, RoutedEventArgs args)
    {
        ...
        try { var result = await new YtDlpMediaProbeService(_toolchain.YtDlpPath).ProbeAsync(uri, GetCookieSelection()); ResultText.Text = result.IsPlaylist ? $"已找到播放清單：{result.Playlist!.Title}（{result.Playlist.Items.Count} 部）" : $"已找到影片：{result.Media!.Title}"; _probedMedia = result.Media ?? result.Playlist?.Items.FirstOrDefault(); QueueButton.IsEnabled = _probedMedia is not null; }
        catch (Exception exception) { ShowError("解析失敗", exception.Message); }
    }

    private async void QueueClicked(object sender, RoutedEventArgs args)
    {
        if (_probedMedia is null) return;
        var request = new DownloadRequest(_probedMedia, _settings.DownloadDirectory, FormatBox.SelectedIndex == 1 ? DownloadFormat.Mp3 : DownloadFormat.Mp4, (VideoQuality)QualityBox.SelectedIndex, GetCookieSelection());
    ...
    private CookieSelection GetCookieSelection() => ((CookieBox.SelectedItem as ComboBoxItem)?.Tag?.ToString()) switch { "chrome" => new(CookieSource.Chrome), "edge" => new(CookieSource.Edge), "firefox" => new(CookieSource.Firefox), _ => CookieSelection.None };
    ...
    private void QueueJobChanged(object? sender, DownloadJob job) => DispatcherQueue.TryEnqueue(async () => { if (job.State is DownloadJobState.Completed or DownloadJobState.Failed or DownloadJobState.Cancelled) await _history.SaveAsync(new(job.Id, job.Request.Media.Extractor, job.Request.Media.MediaId, job.Request.Media.Title, job.Request.Media.SourceUrl, job.Request.Format, job.Request.Quality, job.Request.OutputDirectory, job.State, job.ErrorCategory, job.CreatedAt)); RenderQueue(); });
    ...
    private void ShowError(string title, string message) { StatusBar.Title = title; StatusBar.Message = message; StatusBar.Severity = InfoBarSeverity.Error; }
